...
import { el, openModal, toast, fechaLarga, manejarErrorGuardado } from './dom.js';
import { icons } from './icons.js';
import { fetchFileWithSha, putJsonFile, deleteFile, commitMessage } from './github.js';
import { limpiarCambios, cambiosDe, archivosParaEliminar, limpiarArchivosParaEliminar } from './drafts.js';
...
async function publicarAhora(barrio, close, resolve) {
  ...
  try {
    ...
    // 3. Eliminar archivos marcados (galerías: fotos que el usuario
    //    quitó desde el editor y quedaron pendientes de borrado).
    const paraBorrar = archivosParaEliminar(barrio.id);
    for (const path of paraBorrar) {
      try {
        const f = await fetchFileWithSha(path);
        if (f && f.sha) {
          await deleteFile({
            path,
            message: commitMessage(barrio.nombreVisible, 'foto eliminada de la galería'),
            sha: f.sha
          });
        }
      } catch (err) {
        // Un fallo al borrar un archivo suelto no debe abortar la
        // publicación entera. Se avisa por consola y se sigue.
        console.warn(`No se pudo borrar ${path}:`, err.message);
      }
    }
    limpiarArchivosParaEliminar(barrio.id);

    // 4. Limpiar el estado local de cambios pendientes
