...
import { el, toast, fechaLarga, manejarErrorGuardado } from '../dom.js';
...
function buildForm(id, file) {
  ...
  const swatches = el('div.swatches', { role: 'group', 'aria-label': 'Color en el mapa' });
  ...
  async function onSave() {
    ...
    if (nombre.length < 3) {
      inpNombre.focus();
      toast('El nombre debe tener al menos 3 caracteres.', 'err');
      return;
    }
    const zona = state.zona.trim();
    if (!zona) {
      toast('La zona no puede quedar vacía.', 'err');
      return;
    }
    ...
    try {
      ...
      manejarErrorGuardado(e, { onConflict: showConflictModal });
    } finally {
